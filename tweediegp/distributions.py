...
class FixedDispersionTweedie(Tweedie):
    ...
    def log_prob(self, value):
        ...
        value, mu, rho = broadcast_all(value, self.mu, self.rho)

        theta, k_theta = torch.pow(mu, 1-rho)/(1-rho), torch.pow(mu, 2-rho)/(2-rho)

        return value*theta - k_theta
...
class ZeroInflatedNegativeBinomial(ExponentialFamily):
    ...
    def __init__(self, mu, probs, p_zero, validate_args=None):
        self.mu, self.probs, self.p_zero = broadcast_all(mu, probs, p_zero)
        if isinstance(mu, Number) and isinstance(probs, Number) and isinstance(p_zero, Number):
            batch_shape = torch.Size()
        else:
            batch_shape = self.mu.size()
        super().__init__(batch_shape, validate_args=validate_args)
    ...
    def total_count(self):
        return (self.mu/(1-self.p_zero) -1)*((1-self.probs)/self.probs)

    def log_prob(self, value):
        value = torch.as_tensor(value, dtype=self.mu.dtype, device=self.p_zero.device)
        if self._validate_args:
            self._validate_sample(value)
            
        value, mu, probs, p_zero = broadcast_all(value, self.mu, self.probs, self.p_zero)
        ...
        if torch.any(non_zeros):
            total_count = (mu/(1-p_zero) -1)*((1-probs)/probs)
    ...
    def rsample(self, sample_shape=torch.Size()):
        ...
        mu, probs, p_zero = broadcast_all(self.mu, self.probs, self.p_zero)
        total_count = (mu/(1-p_zero) -1)*((1-probs)/probs)
